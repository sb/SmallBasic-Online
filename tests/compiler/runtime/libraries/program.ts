...
EndFor`);
        ...
        expect(engine.buffer.readValue().toDisplayString()).toBe(`"1"`);
        ...
        engine.execute(ExecutionMode.Debug);
        expect(engine.state).toBe(ExecutionState.BlockedOnOutput);
        expect(engine.buffer.readValue().toDisplayString()).toBe(`"2"`);
        
        engine.execute(ExecutionMode.Debug);
        expect(engine.state).toBe(ExecutionState.BlockedOnOutput);
        expect(engine.buffer.readValue().toDisplayString()).toBe(`"3"`);
